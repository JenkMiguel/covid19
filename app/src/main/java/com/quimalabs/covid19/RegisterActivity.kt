...
class RegisterActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun updateUI(
        ...
        when (uiState) {
            STATE_INITIALIZED -> {
                ...
                enableViews(buttonStartVerification, phoneNumber)
                disableViews(buttonVerifyPhone, buttonResend, fieldVerificationCode)
                detail.text = null
            }
            STATE_CODE_SENT -> {
                // Code sent state, show the verification field, the
                enableViews(buttonVerifyPhone, buttonResend, phoneNumber, fieldVerificationCode)
                disableViews(buttonStartVerification)
                detail.setText(R.string.status_code_sent)
            }
            STATE_VERIFY_FAILED -> {
                // Verification has failed, show all options
                enableViews(buttonStartVerification, buttonVerifyPhone, buttonResend, phoneNumber,
                    fieldVerificationCode)
                detail.setText(R.string.status_verification_failed)
            }
            STATE_VERIFY_SUCCESS -> {
                // Verification has succeeded, proceed to firebase sign in
                disableViews(buttonStartVerification, buttonVerifyPhone, buttonResend, phoneNumber,
                    fieldVerificationCode)
        ...
        if (user == null) {
            ...
            enableViews(phoneNumber, fieldVerificationCode)
    ...
    private fun validatePhoneNumber(): Boolean {
        val cellPhoneNumbers = phoneNumber.text.toString()
        if (TextUtils.isEmpty(cellPhoneNumbers)) {
            phoneNumber.error = "Invalid phone number."
            return false
        }
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.buttonResend -> resendVerificationCode(phoneNumber.text.toString(), resendToken)
